...
class ContinousSliceMaskDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        self.label_map = None
        self.patient_split_char = patient_split_char
    ...
    def load_annotations(self, img_dir, ann_dir, split):
        ...
        image_files = glob(os.path.join(img_dir, '*' + self.img_suffix))
        for image_file in image_files:
            patient_id = image_file.split('/')[-1].split(self.patient_split_char)[0]
            image_id = image_file.split('/')[-1].split('.')[0]
            patient_id2image_id[patient_id].append(image_id)
            ...
            with open(split) as f:
                for line in f:
                    ...
                    for img_id in img_ids:
                        ...
                        img_info['img_info']['filename'] = os.path.join(img_dir, img_id + self.img_suffix)
                        img_info['img_info']['patient_id'] = patient_id
                        img_info['gt_label'] = np.array(gt_label, dtype=np.int64)
                        img_info['img_prefix'] = img_dir
                        if ann_dir is not None:
                            img_info['ann_info'] = dict(seg_map=os.path.join(ann_dir, img_id + self.seg_map_suffix))
